...
- **欄位對應**：上傳後系統會先嘗試依標題文字自動猜測每個系統欄位對應到 Excel 的哪一欄，使用者可自行確認或調整（以欄位字母指定，例如 A 欄／B 欄），不受 Excel 版型影響。自動猜測同時支援中文標題與常見完整英文專利欄位用語（例如 Filing Number、Publication Date、Grant Date、Registered Owner Name 等），但像 `FN` 這類無法穩定判斷語意的縮寫仍猜不到——這是預期行為，欄位字母才是唯一權威依據，猜測只是方便使用者少點幾次下拉選單，猜錯或猜不到都可直接覆寫。在此指定「不比對」的欄位，會自動同步套用到「欄位比對」頁籤，不需重複設定。
...
- 欄位對應的英文標題自動猜測只收錄目前已知的常見用語（`lib/parse-upload.ts` 的 `FIELD_HEADER_ALIASES`），非窮舉所有國外事務所的命名慣例，遇到猜不到的標題屬預期行為，使用者仍需自行以欄位字母確認或調整。
