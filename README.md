...
- **欄位比對**：逐案比對 Excel 內部資料與 TIPO 最新回傳資料是否一致，比對範圍以「欄位對應」頁籤的設定為準；並提供 6 種「忽略差異」選項（大小寫、全形半形、標點符號、多人姓名排列順序、日期格式、專利號國別碼與橫槓），依欄位性質自動套用，減少純格式差異造成的誤判。常見異體字（如「啓」／「啟」）一律視為同一字，不受選項控制。中文多人姓名清單（代理人、申請人中文姓名、發明人中文姓名）無條件把分號、逗號（全形／半形）、頓號都當作人與人的分隔符，不受選項控制——因為內部系統常用逗號或頓號分隔多人，智慧局固定用分號，中文姓名本身不含逗號，這樣拆分不會誤判。英文多人姓名清單（申請人英文姓名、發明人英文姓名）僅以分號辨識人與人邊界，逗號維持原樣不拆分，因為英文姓名本身就是「姓, 名」格式，若把逗號也當分隔符會把同一人的姓名拆成兩截。
- **Excel 匯出**：提供四種報表——狀態分析報表、欄位比對差異報告、TIPO 完整資料重寫檔，以及在原始上傳 Excel 版面中插入標註欄位的標註比對報表：申請案號欄位右邊新增查詢狀態欄（顯示「已從TIPO獲取資訊」或「查無資料或非台灣案」），每個本次選擇比對的欄位右邊各插入一欄比對結果（相符顯示「正確」；不符以紅字顯示智慧局正確值；TIPO 查到案件但該欄位本身無資料時顯示紅字「TIPO無資料」）。
...
- 上傳的 Excel 若有多個工作表（分頁籤），系統只會讀取第一個工作表；其餘工作表的資料不會被解析或查詢。若同一個工作表內有多個表格（例如中間有空白列分隔的兩段資料），系統不會偵測表格邊界，會把第一列視為標題、其餘所有列視為同一段資料，第二個表格的標題列可能被誤讀為資料列。
- 「申請人英文姓名」「發明人英文姓名」為多筆共同申請人／發明人時，若內部系統的 Excel 用逗號、斜線（/）、"and" 等方式分隔多人（而非分號），系統無法可靠拆分成多個人名來比對，會直接視為單一字串整體比對，通常導致判定不符。這是刻意的設計取捨：英文姓名本身是「姓, 名」格式含逗號，無法像中文姓名一樣額外把逗號當分隔符，否則會把同一人的姓名拆散、造成更多誤判；若要正確處理，需要內部 Excel 也統一用分號分隔多位共同申請人／發明人。2026-08-28 業主回饋確認此問題存在但非優先項目，暫不處理。
